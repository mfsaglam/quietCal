...
struct StubCalorieEstimatorTests {
    ...
    @Test func returnsKcalProportionalToGrams() async throws {
        ...
        let kcal = try await estimator.estimate(name: "Anything", grams: 100).calories
        #expect(kcal == 250)
    }

    @Test func ignoresName() async throws {
        let estimator = StubCalorieEstimator(delay: .zero, caloriesPerGram: 1.0)
        let kcalA = try await estimator.estimate(name: "Apple", grams: 100).calories
        let kcalB = try await estimator.estimate(name: "Burger", grams: 100).calories
        #expect(kcalA == kcalB)
    }

    @Test func zeroGramsReturnsZeroKcal() async throws {
        let estimator = StubCalorieEstimator(delay: .zero, caloriesPerGram: 1.5)
        let kcal = try await estimator.estimate(name: "Anything", grams: 0).calories
        #expect(kcal == 0)
    }
    ...
    @Test func truncatesToInt() async throws {
        ...
        let kcal = try await estimator.estimate(name: "X", grams: 13).calories
        #expect(kcal == 19)
    }
}
